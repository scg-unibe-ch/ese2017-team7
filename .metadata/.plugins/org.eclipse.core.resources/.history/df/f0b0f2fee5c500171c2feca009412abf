...
public class CounterService {
	 ...
	 */
	public int countVehiclesInUse(hello.Vehicle vehicleType, Date date) {
		...
		if(vehicleType != null) {
			for(hello.AniOrder order : orders) {
				...
				if(currentVehicle != null && (currentVehicle.getId() == vehicleType.getId())) {
	 ...
	 */
	public boolean checkIfDriverBusy(hello.User driver, Date date) {
		Iterable<hello.AniOrder> orders = orderRepository.findAll();
		boolean busy = false;
		...
		if(driver != null) {
			for(hello.AniOrder order : orders) {
				...
				if(currentDriverId != -1) {
					...
					if(currentDriver != null && (currentDriver.getId() == driver.getId())) {
						if(((date.getTime() - order.getUntil().getTime() < 86400000) && (date.getTime() - order.getUntil().getTime() >= 0)) || ((date.getTime() - order.getUntil().getTime() > -86400000) && (date.getTime() - order.getUntil().getTime() <= 0))) {
							busy = true;
						}	
					}
